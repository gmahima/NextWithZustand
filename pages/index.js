...
const getSampleState = (state) => ({
  bears: state.bears,
  increase: state.increase
});
export default function Home() {
  const { players, highScore, deleteEverything } = usePlayerStore(
    getState,
    shallow
  );
  ...
  return (
    <div
      css={tw`flex flex-col justify-center px-20 md:px-20 space-y-8 text-center h-screen`}
    >
      ...
      <div css={tw`flex flex-col`}>
        ...
        <h1
          css={tw`mt-2 text-black text-2xl border self-center py-1 px-2 rounded bg-green-400 cursor-pointer hover:bg-green-300`}
          onClick={() => {
            increase(10);
          }}
        >
      ...
      <div
        css={tw`bg-white border shadow-lg px-4 w-64 sm:w-2/3 md:px-32 md:w-2/3 h-64 md:h-64 overscroll-y-auto overflow-auto flex flex-col rounded-lg self-center`}
      >
        {players.map((p) => {
